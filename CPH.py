...
coxph = CoxPHModel()
coxph.fit(train[features], train['PFS'], train['disease_progress'])

c_index = concordance_index(coxph, test[features], test['PFS'], test['disease_progress'])
print('C-index: {:.2f}'.format(c_index))

ibs = integrated_brier_score(coxph, test[features], test['PFS'], test['disease_progress'], t_max=84, 
                             figure_size=(20, 6.5))
print('IBS: {:.2f}'.format(ibs))

results = compare_to_actual(coxph, test[features], test['PFS'], test['disease_progress'],
                            is_at_risk = False,  figure_size=(16, 6),
                            metrics = ['rmse', 'mean', 'median'])

coxph_c_index, coxph_brier_score = compute_scores(coxph, test, list(np.arange(0, 86, 2)), features)

coxph_c_index_table_nonutrition = pd.DataFrame(columns=list(np.arange(0, 86, 2)))
series_c_index = pd.Series(coxph_c_index, index = coxph_c_index_table_nonutrition.columns)
coxph_c_index_table_nonutrition = coxph_c_index_table_nonutrition.append(series_c_index, ignore_index=True)
coxph_brier_score_table_nonutrition = pd.DataFrame(columns=coxph_brier_score[0])
series_brier_score = pd.Series(coxph_brier_score[1], index = coxph_brier_score_table_nonutrition.columns)
coxph_brier_score_table_nonutrition = coxph_brier_score_table_nonutrition.append(series_brier_score, ignore_index=True)

coxph_c_index_table_nonutrition.to_csv('coxph_c_index_nonutrition.csv', index=None)
coxph_brier_score_table_nonutrition.to_csv('coxph_brier_score_nonutrition.csv', index=None)


# In[38]:


survival_pred_coxph_nonutrition = {}
for i in range(15):
    survival_pred_coxph_nonutrition[str(i)] = []
    for j in list(np.arange(0, 73, 1)):
        survival_pred_coxph_nonutrition[str(i)].append(coxph.predict_survival(test.iloc[i, :][features], t=j))
...
for i in range(15):
    plt.plot(x, survival_pred_coxph_nonutrition[str(i)], label=('P'+str(i+1)))
plt.xlim([0, 72])
plt.xlabel('Progression-free survival time (months)', fontsize=12, fontweight='bold')
plt.ylabel('Survival probability based on CPH model', fontsize=12, fontweight='bold')
...
risk_prediction_nonutrition_table = pd.DataFrame(data=coxph.predict_risk(test[features]), columns=['risk_prediction'])
risk_prediction_nonutrition_table.to_csv('coxph_risk_prediction_nonutrition.csv', index=None)

test_nonutrition = test.copy()
test_nonutrition['risk_prediction'] =  coxph.predict_risk(test[features])
...
ax.set_xlabel('Time (months)', fontsize=12, fontweight='bold')
ax.set_ylabel('Survival probability based on CPH model', fontsize=12, fontweight='bold')
ax.legend(fontsize=12)

ax.text(10, 0.75, 'p=0.86', fontsize=12, fontweight='bold')
...
coxph = CoxPHModel()
coxph.fit(train[features + nutritional_info], train['PFS'], train['disease_progress'])

c_index = concordance_index(coxph, test[features + nutritional_info], test['PFS'], test['disease_progress'])
print('C-index: {:.2f}'.format(c_index))

ibs = integrated_brier_score(coxph, test[features + nutritional_info], test['PFS'], test['disease_progress'], t_max=84, 
                             figure_size=(20, 6.5) )
print('IBS: {:.2f}'.format(ibs))

results = compare_to_actual(coxph, test[features + nutritional_info], test['PFS'], test['disease_progress'],
                            is_at_risk = False,  figure_size=(16, 6),
                            metrics = ['rmse', 'mean', 'median'])

coxph_c_index, coxph_brier_score = compute_scores(coxph, test, list(np.arange(0, 86, 2)), features + nutritional_info)

coxph_c_index_table_nutrition = pd.DataFrame(columns=list(np.arange(0, 86, 2)))
series_c_index = pd.Series(coxph_c_index, index = coxph_c_index_table_nutrition.columns)
coxph_c_index_table_nutrition = coxph_c_index_table_nutrition.append(series_c_index, ignore_index=True)
coxph_brier_score_table_nutrition = pd.DataFrame(columns=coxph_brier_score[0])
series_brier_score = pd.Series(coxph_brier_score[1], index = coxph_brier_score_table_nutrition.columns)
coxph_brier_score_table_nutrition = coxph_brier_score_table_nutrition.append(series_brier_score, ignore_index=True)

coxph_c_index_table_nutrition.to_csv('coxph_c_index_nutrition.csv', index=None)
coxph_brier_score_table_nutrition.to_csv('coxph_brier_score_nutrition.csv', index=None)


# In[43]:


survival_pred_coxph_nutrition = {}
for i in range(15):
    survival_pred_coxph_nutrition[str(i)] = []
    for j in list(np.arange(0, 73, 1)):
        survival_pred_coxph_nutrition[str(i)].append(coxph.predict_survival(test.iloc[i, :][features + nutritional_info], t=j))
...
plt.figure(figsize=(20,10))
for i in range(15):
    plt.plot(x, survival_pred_coxph_nutrition[str(i)], label=('P'+str(i+1)))
plt.xlim([0, 72])
plt.xlabel('Progression-free survival time (months)', fontsize=12, fontweight='bold')
plt.ylabel('Survival probability based on CPH model', fontsize=12, fontweight='bold')
plt.legend(fontsize=12)
...
risk_prediction_nutrition_table = pd.DataFrame(data=coxph.predict_risk(test[features + nutritional_info]), columns=['risk_prediction'])
risk_prediction_nutrition_table.to_csv('coxph_risk_prediction_nutrition.csv', index=None)

test_nutrition = test.copy()
test_nutrition['risk_prediction'] =  coxph.predict_risk(test[features + nutritional_info])
...
ax.text(10, 0.75, 'p=0.12', fontsize=12, fontweight='bold')
