...
csf.fit(train[features], train['PFS'], train['disease_progress'], max_features=1, max_depth=5, min_node_size=2)

c_index = concordance_index(csf, test[features], test['PFS'], test['disease_progress'])
print('C-index: {:.2f}'.format(c_index))

ibs = integrated_brier_score(csf, test[features], test['PFS'], test['disease_progress'], t_max=84, 
                             figure_size=(20, 6.5))
print('IBS: {:.2f}'.format(ibs))

results = compare_to_actual(csf, test[features], test['PFS'], test['disease_progress'],
...
series_c_index = pd.Series(csf_c_index, index = csf_c_index_table_nonutrition.columns)
csf_c_index_table_nonutrition = csf_c_index_table_nonutrition.append(series_c_index, ignore_index=True)
csf_brier_score_table_nonutrition = pd.DataFrame(columns=csf_brier_score[0])
series_brier_score = pd.Series(csf_brier_score[1], index = csf_brier_score_table_nonutrition.columns)
csf_brier_score_table_nonutrition = csf_brier_score_table_nonutrition.append(series_brier_score, ignore_index=True)

csf_c_index_table_nonutrition.to_csv('csf_c_index_nonutrition.csv', index=None)
csf_brier_score_table_nonutrition.to_csv('csf_brier_score_nonutrition.csv', index=None)


# In[37]:


survival_pred_csf_nonutrition = {}
for i in range(15):
    survival_pred_csf_nonutrition[str(i)] = []
    for j in list(np.arange(0, 73, 1)):
        survival_pred_csf_nonutrition[str(i)].append(csf.predict_survival(test.iloc[i, :][features], t=j))
...
for i in range(15):
    plt.plot(x, survival_pred_csf_nonutrition[str(i)], label=('P'+str(i+1)))
plt.xlim([0, 72])
plt.xlabel('Progression-free survival time (months)', fontsize=12, fontweight='bold')
plt.ylabel('Survival probability based on CSF model', fontsize=12, fontweight='bold')
...
risk_prediction_nonutrition_table = pd.DataFrame(data=csf.predict_risk(test[features]), columns=['risk_prediction'])
risk_prediction_nonutrition_table.to_csv('csf_risk_prediction_nonutrition.csv', index=None)

test_nonutrition = test.copy()
test_nonutrition['risk_prediction'] =  csf.predict_risk(test[features])
...
ax.set_xlabel('Time (months)', fontsize=12, fontweight='bold')
ax.set_ylabel('Survival probability based on CSF model', fontsize=12, fontweight='bold')
ax.legend(fontsize=12)

ax.text(10, 0.65, 'p=0.09', fontsize=12, fontweight='bold')
...
csf.fit(train[features + nutritional_info], train['PFS'], train['disease_progress'], max_features=1, max_depth=5, min_node_size=2)

c_index = concordance_index(csf, test[features + nutritional_info], test['PFS'], test['disease_progress'])
print('C-index: {:.2f}'.format(c_index))

ibs = integrated_brier_score(csf, test[features + nutritional_info], test['PFS'], test['disease_progress'], t_max=84, 
                             figure_size=(20, 6.5))
print('IBS: {:.2f}'.format(ibs))

results = compare_to_actual(csf, test[features + nutritional_info], test['PFS'], test['disease_progress'],
                            is_at_risk = False,  figure_size=(16, 6),
                            metrics = ['rmse', 'mean', 'median'])

csf_c_index, csf_brier_score = compute_scores(csf, test, list(np.arange(0, 86, 2)), features + nutritional_info)

csf_c_index_table_nutrition = pd.DataFrame(columns=list(np.arange(0, 86, 2)))
series_c_index = pd.Series(csf_c_index, index = csf_c_index_table_nutrition.columns)
csf_c_index_table_nutrition = csf_c_index_table_nutrition.append(series_c_index, ignore_index=True)
csf_brier_score_table_nutrition = pd.DataFrame(columns=csf_brier_score[0])
series_brier_score = pd.Series(csf_brier_score[1], index = csf_brier_score_table_nutrition.columns)
csf_brier_score_table_nutrition = csf_brier_score_table_nutrition.append(series_brier_score, ignore_index=True)

csf_c_index_table_nutrition.to_csv('csf_c_index_nutrition.csv', index=None)
csf_brier_score_table_nutrition.to_csv('csf_brier_score_nutrition.csv', index=None)


# In[43]:


survival_pred_csf_nutrition = {}
for i in range(15):
    survival_pred_csf_nutrition[str(i)] = []
    for j in list(np.arange(0, 73, 1)):
        survival_pred_csf_nutrition[str(i)].append(csf.predict_survival(test.iloc[i, :][features + nutritional_info], t=j))
...
plt.figure(figsize=(20,10))
for i in range(15):
    plt.plot(x, survival_pred_csf_nutrition[str(i)], label=('P'+str(i+1)))
plt.xlim([0, 72])
plt.xlabel('Progression-free survival time (months)', fontsize=12, fontweight='bold')
plt.ylabel('Survival probability based on CSF model', fontsize=12, fontweight='bold')
plt.legend(fontsize=12)
...
risk_prediction_nutrition_table = pd.DataFrame(data=csf.predict_risk(test[features + nutritional_info]), columns=['risk_prediction'])
risk_prediction_nutrition_table.to_csv('csf_risk_prediction_nutrition.csv', index=None)

test_nutrition = test.copy()
test_nutrition['risk_prediction'] =  csf.predict_risk(test[features + nutritional_info])
...
ax.text(10, 0.75, 'p=0.37', fontsize=12, fontweight='bold')
